  ...
  return <header className="sticky top-0 z-50 border-b bg-[#f6f1e7]/95 backdrop-blur">
    <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3 sm:px-5">
      <Link href={`/${lang}`} className="flex items-center gap-3" aria-label="Sensus Vitae">
        ...
        <span className="hidden sm:block"><span className="font-editorial block text-xl font-bold tracking-[.08em]">SENSUS VITAE</span><span className="hidden text-xs tracking-[.16em] text-muted-foreground lg:block">SAME QUESTIONS. DEEPER ANSWERS.</span></span>
      </Link>
      ...
      <div className="flex items-center gap-0.5 sm:gap-1">{(["ru", "es", "en", "nl"] as const).map((code) => <Link key={code} href={languageHref(code)} className={`rounded-full px-2 py-1.5 text-xs font-bold uppercase sm:px-2.5 ${code === lang ? "bg-primary text-primary-foreground" : "hover:bg-secondary"}`}>{code}</Link>)}<Link href="/studio" className="ml-0.5 rounded-full p-2 text-muted-foreground hover:bg-secondary hover:text-foreground sm:ml-1" aria-label="Редакторская панель"><PenLine size={17}/></Link></div>
    </div>
    <nav className="mx-auto flex max-w-6xl gap-2 overflow-x-auto px-4 pb-3 text-sm font-medium md:hidden" aria-label="Primary navigation">{navItems.map((item) => <Link key={item.href} href={item.href} aria-current={item.active ? "page" : undefined} className={`shrink-0 rounded-full px-3 py-1.5 ${item.active ? "bg-[#e5dcc8] text-[#174f3c]" : "hover:bg-[#eee6d6]"}`}>{item.label}</Link>)}</nav>
  </header>;
}
