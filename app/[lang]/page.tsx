...
export default async function LanguageHome({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return <main>
    ...
    <section className="border-y bg-[#ece5d7]/55"><div className="mx-auto max-w-6xl px-5 py-14"><div className="grid gap-8 lg:grid-cols-[.65fr_1.35fr]">
      ...
      <div className="grid gap-px overflow-hidden rounded-[1.4rem] border bg-border sm:grid-cols-2">{categories[lang].map(([title, description], i) => { const Icon = icons[i]; const content = <><Icon size={21} className="mb-4 text-[#a17a34]"/><h3 className="font-editorial text-xl font-bold">{title}</h3><p className="mt-2 leading-6 text-[#607068]">{description}</p>{i === 0 && <span className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-[#174f3c]">{t.openPath}<ArrowRight size={16}/></span>}</>; return i === 0 ? <Link key={title} href={`/${lang}/psychobiology`} aria-label={`${t.openPath}: ${title}`} className="group bg-[#f9f5ec] p-6 transition hover:bg-[#fffaf0] focus-visible:z-10 focus-visible:outline-2 focus-visible:outline-offset-[-3px] focus-visible:outline-[#b48a39] [&_svg]:transition-transform [&:hover_svg]:translate-x-0.5">{content}</Link> : <div key={title} className="bg-[#f9f5ec] p-6">{content}</div>; })}</div>
    </div></div></section>
